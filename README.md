# Satellite Tracker Analysis - Interactive Orbital Mechanics Visualization

## A Python Flask-based web application for real-time satellite orbital calculations and 3D trajectory visualization using SGP4 propagation

## Overview

Satellite Tracker Analysis is a comprehensive web application that combines the power of Python's orbital mechanics libraries with an interactive browser-based interface. This tool enables users to visualize satellite trajectories in real-time, analyze altitude profiles over time, and experiment with Keplerian orbital elements to understand satellite motion.
...
- Interactive 3D orbital trajectory visualization with Earth reference
...
*The main interface showing altitude graph, 3D orbit visualization, and control panel*
...
bashpython -c "import flask, numpy, sgp4; print('All packages installed successfully')"
Usage Instructions
...
Obtain TLE data from sources like:
...
Paste the two TLE lines into the input fields in the right panel
Click "Load from TLE" to automatically extract orbital parameters
...
Solution: Ensure all required packages are installed using pip install flask numpy sgp4
...
JavaScript: Handles user interactions, API calls, and Plotly visualizations
...
Semi-Major Axis (a): Half the longest diameter of the orbital ellipse. Determines orbital period.
Eccentricity (e): Shape of the orbit. 0 is circular, values approaching 1 are highly elliptical.
Inclination (i): Tilt of orbital plane relative to Earth's equator. 0° is equatorial, 90° is polar.
Right Ascension of Ascending Node (RAAN/Ω): Orientation of the orbital plane in space.
Argument of Perigee (ω): Orientation of the ellipse within the orbital plane.
Mean Anomaly (M): Position of satellite along its orbit at epoch time.
...
Aerospace Engineering Students: Visualizing theoretical orbital mechanics concepts
...
Research: Analyzing orbital decay, station-keeping requirements, or mission planning
...
Verify your Python and package versions meet requirements
Ensure TLE data is correctly formatted
Check browser console for JavaScript errors

Version History

v1.0: Initial release with SGP4 propagation, 3D visualization, and TLE parsing
...
Install dependencies: pip install flask numpy sgp4
Run application: python app.py
Open browser to http://localhost:5001
Click "Update Visualization" to see ISS orbit
Experiment with sliders to modify orbital parameters
