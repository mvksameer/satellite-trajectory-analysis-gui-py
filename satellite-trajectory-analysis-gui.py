...
def calculate_altitude_from_tle(tle_line_1, tle_line_2):
    ...
    try:
        ...
        # FIX 1: Pull the exact epoch directly from the TLE
        jd = satellite.jdsatepoch
        fr = satellite.jdsatepochF
        
        for t in times:
            # Convert the elapsed seconds 't' into days and add to the fractional Julian day
            fr_t = fr + (t / 86400.0)
            
            # Use SGP4 relative to the exact TLE epoch
            error, r, v = satellite.sgp4(jd, fr_t)
            
            if error == 0:  
                distance = (r[0]**2 + r[1]**2 + r[2]**2)**0.5
                altitude = distance - 6378.135  # Earth's equatorial radius in km
                altitudes.append(altitude)
                time_hours.append(t / 3600)  
...
def calculate_3d_orbit_keplerian(a, e, i, Omega, w, M0, num_points=300):
    ...
    for j in range(num_points):
        ...
        # FIX 2: Solve Kepler's equation using TRUE Newton-Raphson
        E = M
        for _ in range(10):  
            # Formula: E_{new} = E_{old} - f(E) / f'(E)
...
<body>
    <div class="header">
        <h1> Satellite Tracker Analysis</h1>
...
def calculate():
    ...
    try:
        ...
        # Calculate altitude data using your fixed SGP4 math
        times, altitudes = calculate_altitude_from_tle(tle_line_1, tle_line_2)
        
        # Get Keplerian elements from the sliders
        ...
        # Calculate 3D orbit using your fixed Newton-Raphson math
...
if __name__ == '__main__':
    # Hardcoding a safe port to avoid conflicts
    port = 5050
    ...
    # host='127.0.0.1' and use_reloader=False prevent port blocking and firewalls
